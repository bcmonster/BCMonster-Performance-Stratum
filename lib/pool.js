...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function OnBlockchainSynced(syncedCallback) {
        var callConfig = {
            ...
            "rules": ["segwit"]  // Required by Bitcoin Core since 0.13.0
        };

        // Removed conditional - SegWit is always required for Bitcoin
    ...
    function GetBlockTemplate(callback) {
        ...
        function getBlockSubsidyandTemplate(blockheight) {
            // Bitcoin Core doesn't have getblocksubsidy (that's a Zcash/altcoin RPC)
            // For Bitcoin, coinbase value is in the block template itself
            if (options.coin.getblocksubsidy === false || options.coin.name === 'bitcoin') {
                // Skip subsidy call for Bitcoin - template has coinbasevalue
                getBlockTemplate(null);
            } else {
                // Other coins (Zcash, etc.) need getblocksubsidy
                _this.daemon.cmd(
                    'getblocksubsidy',
                    [blockheight],
        ...
        function getBlockTemplate(subsidy) {
            ...
            async function (result) {
                    if (result.error) {
                        ...
                        // For Bitcoin, subsidy is null - coinbasevalue is in the template
                        if (subsidy && subsidy.miner) {
                            result.response.miner = subsidy.miner;
                        } else if (result.response.coinbasevalue) {
                            // Bitcoin: use coinbasevalue from template
                            result.response.miner = result.response.coinbasevalue;
                        }

                        if (options.coin.vFundingStreams && subsidy) {
                            //Zcash uses fundingstreams instead of founders reward
                            result.response.fundingstreams = subsidy.fundingstreams
                        } else if (subsidy && !result.response.founders)
                        {
                            ...
                            // founders already set in block template or not applicable
                        }
                        ...
                        // All these are altcoin-specific fields that don't exist for Bitcoin
                        if (subsidy) {
